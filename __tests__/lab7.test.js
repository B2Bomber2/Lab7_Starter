describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // button inside the shadowRoot
    const shadowRootButton = await product.evaluateHandle(el => el.shadowRoot.querySelector('button'));

    // click the button
    await shadowRootButton.click();

    // get the text from button click
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < products.length; i++){
      ...
      // make sure I am clicking the right thing
      const buttonText = await shadowRootButton.evaluate(button => button.innerText);
